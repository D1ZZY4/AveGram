package org.avegram.ave.translate
...
import org.avegram.ave.translate.source.*
import org.avegram.ave.ui.PopupBuilder
import org.avegram.ave.utils.AppScope
import org.avegram.ave.utils.receiveLazy
...
interface Translator {
    ...
    companion object {
        ...
        fun getInputTranslateLangForChat(chatId: Long): String {
            ...
            return AveConfig.getPreferences().getString(key, null)
                ?: AveConfig.translateInputLang.String()
        }

        @JvmStatic
        fun setInputTranslateLangForChat(chatId: Long, langCode: String) {
            val key = "translateInputLang_$chatId"
            AveConfig.getPreferences().edit {
        ...
        suspend fun translate(to: Locale, query: String, provider: Int = 0): String {
            val result: TLRPC.TL_textWithEntities = translateBase(to, query, ArrayList(), provider.takeIf { it != 0 } ?: AveConfig.translationProvider.Int())
        ...
        suspend fun translate(to: Locale, query: String, entities: ArrayList<TLRPC.MessageEntity>, provider: Int = 0): TLRPC.TL_textWithEntities {
            val result: TLRPC.TL_textWithEntities = translateBase(to, query, entities, provider.takeIf { it != 0 } ?: AveConfig.translationProvider.Int())
            return result
        }
        ...
        @JvmOverloads
        fun translate(
            to: Locale = AveConfig.translateToLang.String()?.code2Locale
                ?: LocaleController.getInstance().currentLocale,
            query: String,
            provider: Int = 0,
            ...
            AppScope.io.launch {
                runCatching {
                    val result: String = translate(to, query, provider.takeIf { it != 0 } ?: AveConfig.translationProvider.Int())
            ...
            AppScope.io.launch {
                runCatching {
                    val result = translateBase(
                        to, query, entities, AveConfig.translationProvider.Int()
                    )
        ...
        fun translateWithContext(
            ...
            AppScope.io.launch {
                runCatching {
                    val effectiveProvider = provider.takeIf { it != 0 } ?: AveConfig.translationProvider.Int()
                ...
                runCatching {
                    ...
                    if (query.question != null) {
                        translatedPoll.question = translateBase(
                            to, query.question.text, ArrayList(), AveConfig.translationProvider.Int()
                        )
                    }
                    for (answer in query.answers) {
                        var translatedAnswer = TLRPC.TL_pollAnswer()
                        translatedAnswer.text = translateBase(
                            to, answer.text.text, ArrayList(), AveConfig.translationProvider.Int()
                        )
                    ...
                    if (query.solution != null) {
                        translatedPoll.solution = translateBase(
                            to, query.solution.text, ArrayList(), AveConfig.translationProvider.Int()
                        )
                    }
        ...
        suspend fun translateArticle(query: String) = translateArticle(
            AveConfig.translateToLang.String()?.code2Locale
        ...
        suspend fun translateArticle(to: Locale, query: String): String {
            val provider = if (NaConfig.enableSeparateArticleTranslator.Bool()) {
                ...
                AveConfig.translationProvider.Int()
            }
