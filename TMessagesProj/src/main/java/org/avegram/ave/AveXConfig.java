...
public class AveXConfig {
    ...
    public static SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("avex_config", Context.MODE_PRIVATE);
    ...
    public static void showCustomApiBottomSheet(BaseFragment fragment) {
        ...
        builder.addRadioItem(getString(R.string.CustomApiNo), AveXConfig.customApi != API_TYPE_CUSTOM, (cell) -> {
        ...
        builder.addRadioItem(getString(R.string.CustomApiInput), AveXConfig.customApi == API_TYPE_CUSTOM, (cell) -> {
        ...
        if (AveXConfig.customAppId != 0) {
            inputs[0].setText(String.valueOf(AveXConfig.customAppId));
        }

        inputs[1] = builder.addEditText("App Hash");
        if (!TextUtils.isEmpty(AveXConfig.customAppHash)) {
            inputs[1].setText(AveXConfig.customAppHash);
        }

        if (AveXConfig.customApi != API_TYPE_CUSTOM) {
            for (EditText input : inputs) {
                input.setVisibility(View.GONE);
            }
        }
        ...
        builder.addButton(getString(R.string.Set), true, (it) -> {
            ...
            if (targetType == -1) {
                targetType = AveXConfig.customApi == API_TYPE_CUSTOM ? API_TYPE_CUSTOM : API_TYPE_DEFAULT;
            }
            ...
            if (targetType == API_TYPE_CUSTOM) {
                ...
                AveXConfig.customApi = API_TYPE_CUSTOM;
                AveXConfig.customAppId = Integer.parseInt(appIdStr);
                AveXConfig.customAppHash = appHashStr;
    ...
    private static void resetCustomApi() {
        AveXConfig.customApi = API_TYPE_DEFAULT;
        AveXConfig.customAppId = 0;
        AveXConfig.customAppHash = "";
    }

    private static void saveCustomApiAndRestart(Context context) {
        AveXConfig.saveCustomApi();
        AlertDialog restart = new AlertDialog(context, 0);
        restart.setTitle(getString(R.string.AveGram));
