...
import org.avegram.ave.config.cell.AbstractConfigCell;
import org.avegram.ave.config.cell.ConfigCellCheckBox;
import org.avegram.ave.config.cell.ConfigCellCustom;
import org.avegram.ave.config.cell.ConfigCellDivider;
import org.avegram.ave.config.cell.ConfigCellHeader;
import org.avegram.ave.config.cell.ConfigCellSelectBox;
import org.avegram.ave.config.cell.ConfigCellTextCheck;
import org.avegram.ave.config.cell.ConfigCellTextCheck2;
import org.avegram.ave.config.cell.ConfigCellTextCheckIcon;
import org.avegram.ave.config.cell.ConfigCellTextInput;
import org.avegram.ave.helpers.TranscribeHelper;
import org.avegram.ave.helpers.remote.EmojiHelper;
import org.avegram.ave.ui.PopupBuilder;
import org.avegram.ave.ui.cells.EmojiSetCell;
import org.avegram.ave.ui.cells.StickerSizePreviewMessagesCell;
import org.avegram.NaConfig;
import org.avegram.helper.DoubleTap;

@SuppressLint("RtlHardcoded")
@SuppressWarnings({"unused", "FieldCanBeLocal"})
public class AveChatSettingsActivity extends BaseAveXSettingsActivity implements NotificationCenter.NotificationCenterDelegate, EmojiHelper.EmojiPacksLoadedListener {
    ...
    private final AbstractConfigCell hideTimeForStickerRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.hideTimeForSticker));
    ...
    private final AbstractConfigCell unreadBadgeOnBackButton = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.unreadBadgeOnBackButton));
    private final AbstractConfigCell sendCommentAfterForwardRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.sendCommentAfterForward));
    private final AbstractConfigCell useChatAttachMediaMenuRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.useChatAttachMediaMenu, getString(R.string.UseChatAttachEnterMenuNotice)));
    ...
    private final AbstractConfigCell disableLinkPreviewByDefaultRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.disableLinkPreviewByDefault));
    ...
    private final AbstractConfigCell showSeconds = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.showSeconds));
    ...
    private final AbstractConfigCell disableTrendingRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.disableTrending));
    ...
    private final AbstractConfigCell takeGIFasVideoRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.takeGIFasVideo));
    private final AbstractConfigCell autoPauseVideoRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.autoPauseVideo, getString(R.string.AutoPauseVideoAbout)));
    ...
    private final AbstractConfigCell showSpoilersDirectlyRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.showSpoilersDirectly));
    ...
    private final AbstractConfigCell dontSendGreetingStickerRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.dontSendGreetingSticker));
    private final AbstractConfigCell hideGroupStickerRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.hideGroupSticker));
    ...
    private final AbstractConfigCell messageMenuRow = cellGroup.appendCell(new ConfigCellTextCheckIcon(null, "MessageMenu", null, R.drawable.msg_list, false, () ->
            showDialog(showConfigMenuWithIconAlert(this, R.string.MessageMenu, new ArrayList<>() {{
                ...
                add(new ConfigCellTextCheckIcon(AveConfig.showAddToSavedMessages, getString(R.string.AddToSavedMessages), R.drawable.msg_saved));
                ...
                add(new ConfigCellTextCheckIcon(AveConfig.showRepeat, getString(R.string.Repeat), R.drawable.msg_repeat));
                add(new ConfigCellTextCheckIcon(NaConfig.INSTANCE.getShowRepeatAsCopy(), R.drawable.msg_repeat));
                add(new ConfigCellTextCheckIcon(AveConfig.showDeleteDownloadedFile, getString(R.string.DeleteDownloadedFile), R.drawable.msg_clear));
                add(new ConfigCellTextCheckIcon(AveConfig.showViewHistory, getString(R.string.ViewHistory), R.drawable.menu_recent));
                add(new ConfigCellTextCheckIcon(AveConfig.showTranslate, getString(R.string.Translate), R.drawable.msg_translate));
                add(new ConfigCellTextCheckIcon(NaConfig.INSTANCE.getShowTranslateMessageLLM(), R.drawable.magic_stick_solar));
                add(new ConfigCellTextCheckIcon(AveConfig.showShareMessages, getString(R.string.ShareMessages), R.drawable.msg_shareout));
                add(new ConfigCellTextCheckIcon(AveConfig.showMessageHide, getString(R.string.Hide), R.drawable.msg_disable));
                add(new ConfigCellTextCheckIcon(AveConfig.showReport, getString(R.string.ReportChat), R.drawable.msg_report));
                add(new ConfigCellTextCheckIcon(AveConfig.showAdminActions, getString(R.string.EditAdminRights), R.drawable.profile_admin));
                add(new ConfigCellTextCheckIcon(AveConfig.showChangePermissions, getString(R.string.ChangePermissions), R.drawable.msg_permissions));
                add(new ConfigCellTextCheckIcon(AveConfig.showMessageDetails, getString(R.string.MessageDetails), R.drawable.msg_info));
            }}))
    ));
    ...
    private final AbstractConfigCell hideKeyboardOnChatScrollRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.hideKeyboardOnChatScroll));
    private final AbstractConfigCell disableVibrationRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.disableVibration));
    ...
    private final AbstractConfigCell disableProximityEventsRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.disableProximityEvents));
    private final AbstractConfigCell rememberAllBackMessagesRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.rememberAllBackMessages));
    ...
    private final AbstractConfigCell hideSendAsChannelRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.hideSendAsChannel));
    ...
    private final AbstractConfigCell disableSwipeToNextRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.disableSwipeToNext));
    ...
    private final AbstractConfigCell skipOpenLinkConfirmRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.skipOpenLinkConfirm));
    private final AbstractConfigCell askBeforeCallRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.askBeforeCall));
    private final AbstractConfigCell repeatConfirmRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.repeatConfirm));
    ...
    private final AbstractConfigCell confirmAVRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.confirmAVMessage));
    ...
    public AveChatSettingsActivity() {
    ...
    public View createView(Context context) {
        ...
        menuItem.setVisibility(AveConfig.stickerSize.Float() != 14.0f ? View.VISIBLE : View.GONE);
        ...
        cellGroup.callBackSettingsChanged = (key, newValue) -> {
            if (key.equals(AveConfig.disableProximityEvents.getKey())) {
                MediaController.getInstance().recreateProximityWakeLock();
            } else if (key.equals(AveConfig.showSeconds.getKey())) {
                tooltip.showWithAction(0, UndoView.ACTION_NEED_RESTART, null, null);
            } else if (key.equals(NaConfig.INSTANCE.getConfirmAllLinks().getKey())) {
                checkSkipOpenLinkConfirmRows();
            } else if (key.equals(AveConfig.useChatAttachMediaMenu.getKey())) {
                checkConfirmAVRows();
            ...
            } else if (key.equals(AveConfig.hideTimeForSticker.getKey())) {
                stickerSizeCell.invalidate();
    ...
    protected void onActionBarItemClick(int id) {
        if (id == 1) {
            AveConfig.stickerSize.setConfigFloat(14.0f);
    ...
    protected void onCustomCellClick(View view, int position, float x, float y) {
        if (position == cellGroup.rows.indexOf(maxRecentStickerCountRow)) {
            ...
            builder.setItems(types, (i, str) -> {
                AveConfig.maxRecentStickerCount.setConfigInt(Integer.parseInt(str.toString()));
            ...
            presentFragment(new AveEmojiSettingsActivity());
    ...
    private class StickerSizeCell extends FrameLayout {
        ...
        public StickerSizeCell(Context context) {
            ...
            sizeBar.setDelegate((stop, progress) -> {
                AveConfig.stickerSize.setConfigFloat(startStickerSize + (endStickerSize - startStickerSize) * progress);
                StickerSizeCell.this.invalidate();
                menuItem.setVisibility(View.VISIBLE);
            });
            addView(sizeBar, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 38, Gravity.LEFT | Gravity.TOP, 9, 5, 43, 11));

            messagesCell = new StickerSizePreviewMessagesCell(context, AveChatSettingsActivity.this);
        ...
        protected void onDraw(Canvas canvas) {
            ...
            canvas.drawText("" + Math.round(AveConfig.stickerSize.Float()), getMeasuredWidth() - AndroidUtilities.dp(39), AndroidUtilities.dp(28), textPaint);
        }

        @Override
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            super.onMeasure(widthMeasureSpec, heightMeasureSpec);
            sizeBar.setProgress((AveConfig.stickerSize.Float() - startStickerSize) / (float) (endStickerSize - startStickerSize));
        }
    ...
    private class ListAdapter extends BaseListAdapter {
        ...
        protected void onBindCustomViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            if (holder.itemView instanceof TextSettingsCell textCell) {
                if (position == cellGroup.rows.indexOf(maxRecentStickerCountRow)) {
                    textCell.setTextAndValue(getString(R.string.maxRecentStickerCount), String.valueOf(AveConfig.maxRecentStickerCount.Int()), true);
    ...
    private void checkConfirmAVRows() {
        boolean useChatAttachMediaMenu = AveConfig.useChatAttachMediaMenu.Bool();
        if (listAdapter == null) {
