...
import org.avegram.ave.config.cell.ConfigCellText;
import org.avegram.ave.config.cell.ConfigCellTextCheck;
import org.avegram.ave.config.cell.ConfigCellTextDetail;
import org.avegram.ave.config.cell.ConfigCellTextInput;
import org.avegram.ave.config.cell.ConfigCellTextInput2;
import org.avegram.ave.utils.AndroidUtil;
import org.avegram.NaConfig;

@SuppressWarnings({"unused", "FieldCanBeLocal"})
public class AveGeneralSettingsActivity extends BaseAveXSettingsActivity {
    ...
    private final AbstractConfigCell disableNumberRoundingRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.disableNumberRounding, "4.8K -> 4777"));
    ...
    private final AbstractConfigCell usePersianCalendarRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.usePersianCalendar, getString(R.string.UsePersianCalendarInfo)));
    private final AbstractConfigCell displayPersianCalendarByLatinRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.displayPersianCalendarByLatin));
    ...
    private final AbstractConfigCell nameOrderRow = cellGroup.appendCell(new ConfigCellSelectBox(null, AveConfig.nameOrder, new String[]{
    ...
    private final AbstractConfigCell useIPv6Row = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.useIPv6));
    ...
    private final AbstractConfigCell dnsTypeRow = cellGroup.appendCell(new ConfigCellSelectBox(null, AveConfig.dnsType, new String[]{
            getString(R.string.MapPreviewProviderTelegram),
            getString(R.string.AveGram),
            getString(R.string.DnsTypeSystem),
            getString(R.string.CustomDoH),
    }, null));
    private final AbstractConfigCell customDoHRow = cellGroup.appendCell(new ConfigCellTextInput2(null, AveConfig.customDoH, "https://1.0.0.1/dns-query, https://...", null));
    ...
    private final AbstractConfigCell useOSMDroidMapRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.useOSMDroidMap));
    private final AbstractConfigCell mapDriftingFixForGoogleMapsRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.mapDriftingFixForGoogleMaps));
    private final AbstractConfigCell mapPreviewRow = cellGroup.appendCell(new ConfigCellSelectBox(null, AveConfig.mapPreviewProvider, new String[]{
            getString(R.string.MapPreviewProviderTelegram),
    ...
    private final AbstractConfigCell hidePhoneRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.hidePhone));
    ...
    private final AbstractConfigCell disableSystemAccountRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.disableSystemAccount));
    ...
    private final AbstractConfigCell disableNotificationBubblesRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.disableNotificationBubbles));
    ...
    private final AbstractConfigCell win32Row = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.disableAutoDownloadingWin32Executable));
    private final AbstractConfigCell archiveRow = cellGroup.appendCell(new ConfigCellTextCheck(AveConfig.disableAutoDownloadingArchive));
    private final AbstractConfigCell dividerAutoDownload = cellGroup.appendCell(new ConfigCellDivider());

    public AveGeneralSettingsActivity() {
    ...
    public View createView(Context context) {
        ...
        cellGroup.callBackSettingsChanged = (key, newValue) -> {
            if (key.equals(AveConfig.disableSystemAccount.getKey())) {
                if ((boolean) newValue) {
            ...
            } else if (key.equals(AveConfig.useOSMDroidMap.getKey())) {
                checkMapDriftingFixRows();
                ...
                checkCustomTitleRows();
                tooltip.showWithAction(0, UndoView.ACTION_NEED_RESTART, null, null);
            } else if (key.equals(AveConfig.usePersianCalendar.getKey())) {
                tooltip.showWithAction(0, UndoView.ACTION_NEED_RESTART, null, null);
            } else if (key.equals(AveConfig.dnsType.getKey())) {
                checkCustomDoHRows();
                tooltip.showWithAction(0, UndoView.ACTION_NEED_RESTART, null, null);
    ...
    private void checkCustomDoHRows() {
        boolean useDoH = AveConfig.dnsType.Int() == AveConfig.DNS_TYPE_CUSTOM_DOH;
        if (listAdapter == null) {
            if (!useDoH) {
    ...
    private void checkMapDriftingFixRows() {
        boolean useOSMDroid = AveConfig.useOSMDroidMap.Bool();
        if (listAdapter == null) {
            if (useOSMDroid) {
    ...
    private String sanitizeCustomSavePath(String input) {
        ...
        return (String) AveConfig.customSavePath.defaultValue;
    }
}
