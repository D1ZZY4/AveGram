...
import org.avegram.ave.helpers.CloudSettingsHelper;
...
public class AveSettingsActivity extends BaseAveSettingsActivity {
        ...
        actionBar.setActionBarMenuOnItemClick(new ActionBar.ActionBarMenuOnItemClick() {
            ...
            public void onItemClick(int id) {
                if (id == -1) {
                    ...
                    CloudSettingsHelper.getInstance().showDialog(AveSettingsActivity.this);
                }
            }
        });
    ...
    protected String getActionBarTitle() {
        return getString(R.string.AveSettings);
    }
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position == chatRow) {
            presentFragment(new AveChatSettingsActivity());
        } else if (position == generalRow) {
            presentFragment(new AveGeneralSettingsActivity());
        } else if (position == appearanceRow) {
            presentFragment(new AveAppearanceSettingsActivity());
        } else if (position == ayuMomentsRow) {
            presentFragment(new AveAyuMomentsSettingsActivity());
        } else if (position == passcodeRow) {
            presentFragment(new AvePasscodeSettingsActivity());
        } else if (position == experimentRow) {
            presentFragment(new AveExperimentalSettingsActivity());
        } else if (position == translatorRow) {
            presentFragment(new AveTranslatorSettingsActivity());
        } else if (position == aboutRow) {
            presentFragment(new AveAboutActivity());
            ...
            AlertUtil.showConfirm(getParentActivity(),
                    ...
                    true,
                    () -> {
                        ApplicationLoader.applicationContext.getSharedPreferences("avecloud", Activity.MODE_PRIVATE).edit().clear().commit();
                        ApplicationLoader.applicationContext.getSharedPreferences("avex_config", Activity.MODE_PRIVATE).edit().clear().commit();
                        AveConfig.getPreferences().edit().clear().commit();
            ...
            switch (viewType) {
                ...
                case TYPE_TEXT: {
                    ...
                    if (position == chatRow) {
                        ...
                        textCell.setTextAndIcon(getString(R.string.PasscodeAve), R.drawable.msg_permissions, true);
                    } else if (position == experimentRow) {
    ...
    public void onActivityResultFragment(int requestCode, int resultCode, Intent data) {
        if (requestCode == 21 && resultCode == Activity.RESULT_OK && data != null) {
            ...
            if (uri != null) {
                ...
                String tempFile = UUID.randomUUID().toString().replace("-", "") + ".avex-settings.json";
