...
public class AveEmojiSettingsActivity extends BaseAveSettingsActivity implements EmojiHelper.EmojiPackLoadListener, EmojiHelper.EmojiPacksLoadedListener, ChatAttachAlertDocumentLayout.DocumentSelectActivityDelegate {
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position >= emojiPacksStartRow && position < emojiPacksEndRow) {
            ...
            AveConfig.useSystemEmoji.toggleConfigBool();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(AveConfig.useSystemEmoji.Bool());
            }
            EmojiHelper.reloadEmoji();
            listAdapter.notifyEmojiSetsChanged();
        } else if (position == customEmojiAddRow) {
            chatAttachAlert = new ChatAttachAlert(getParentActivity(), AveEmojiSettingsActivity.this, false, false);
            ...
            switch (holder.getItemViewType()) {
                case TYPE_CHECK: {
                    ...
                    if (position == useSystemEmojiRow) {
                        if (partial) {
                            textCheckCell.setChecked(AveConfig.useSystemEmoji.Bool());
                        } else {
                            textCheckCell.setTextAndCheck(getString(R.string.EmojiUseDefault), AveConfig.useSystemEmoji.Bool(), false);
                        }
                    }
                    break;
                }
                ...
                case TYPE_EMOJI_SELECTION: {
                    ...
                    if (emojiPackInfo != null) {
                        emojiPackSetCell.setChecked(!hasSelected() && emojiPackInfo.getPackId().equals(EmojiHelper.getInstance().getSelectedEmojiPackId()) && !AveConfig.useSystemEmoji.Bool(), partial);
        ...
        private void processSelectionMenu(int which) {
            ...
            if (count > 1) {
                ...
                if (which == menu_share) {
                    ...
                    EmojiHelper.getInstance().cancelableDelete(AveEmojiSettingsActivity.this, pack, new EmojiHelper.OnBulletinAction() {
                        @Override
