...
public class DatacenterActivity extends BaseAveSettingsActivity implements NotificationCenter.NotificationCenterDelegate {
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position > datacentersRow && position < datacenters2Row) {
            var datacenterInfo = AveConfig.datacenterInfos.get(position - datacentersRow - 1);
    ...
    protected void updateRows() {
        ...
        for (var datacenterInfo : AveConfig.datacenterInfos) {
            checkDatacenter(datacenterInfo, false);
        }
    }

    private void checkDatacenter(AveConfig.DatacenterInfo datacenterInfo, boolean force) {
        if (datacenterInfo.checking) {
            return;
        }
        ...
        int position = datacentersRow + AveConfig.datacenterInfos.indexOf(datacenterInfo) + 1;
        if (force) {
            listAdapter.notifyItemChanged(position);
        }
        datacenterInfo.pingId = ConnectionsManager.getInstance(currentAccount).checkProxy("ping.ave", datacenterInfo.id, null, null, null, time -> AndroidUtilities.runOnUIThread(() -> {
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            switch (holder.getItemViewType()) {
                ...
                case Integer.MAX_VALUE: {
                    ...
                    cell.setDC(AveConfig.datacenterInfos.get(position - datacentersRow - 1), position + 1 != datacenters2Row);
                    break;
                }
            }
        }
    ...
    public class DatacenterCell extends FrameLayout {
        ...
        private AveConfig.DatacenterInfo currentInfo;
        ...
        public void setDC(AveConfig.DatacenterInfo info, boolean divider) {
