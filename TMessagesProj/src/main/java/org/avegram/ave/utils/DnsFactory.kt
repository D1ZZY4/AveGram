...
object DnsFactory {
    ...
    private fun providers(): Array<String> {
        if (AveConfig.dnsType.Int() != AveConfig.DNS_TYPE_CUSTOM_DOH) {
            return DEFAULT_DOH_PROVIDERS
        }

        val validCustomProviders = AveConfig.customDoH.String()
            .split(",")
            .map { it.trim() }
            .filter { url ->
    ...
    fun lookup(domain: String, fallback: Boolean = false): List<InetAddress> {
        if (AveConfig.dnsType.Int() != AveConfig.DNS_TYPE_SYSTEM) {
        ...
        try {
            ...
            val determinedStrategy = when {
                hasIPv6 -> when {
                    ...
                    AveConfig.useIPv6.Bool() -> USE_IPV6_ONLY
    ...
    private fun getDnsQueryType(fallback: Boolean): Int {
        ...
        return if (noFallback) {
            ...
            when (strategy) {
                ...
                USE_IPV4_IPV6_RANDOM -> if (AveConfig.useIPv6.Bool() xor !fallback) Type.A else Type.AAAA
                else -> Type.A
            }
        }
    }
