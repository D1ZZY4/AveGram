...
public class AyuFilter {
    ...
    public static boolean isIgnoredBlockedMessage(MessageObject msg) {
        if (msg == null || msg.isOutOwner() || !AveConfig.ignoreBlocked.Bool()) {
            return false;
        }
    ...
    public static boolean isBlockedChannel(long dialogId) {
        return AveConfig.ignoreBlocked.Bool() && getBlockedChannels().contains(dialogId);
    }

    public static boolean isCustomFilteredPeer(long peerId) {
        return AveConfig.ignoreBlocked.Bool() && peerId > 0L && getCustomFilteredUsers().contains(peerId);
    }
